...
import stiefel.stiefel_optimizer 
...
class RKM_Stiefel(nn.Module):
    ...
    def forward(self, x, trained_phi=None):
        ...
        #     # Check if eval_centering_term has previously been computed
        #     if self.eval_centering_term is not None:
        #         self.centering_term = self.eval_centering_term
        #     elif trained_phi is not None:
        #         eval_D = trained_phi @ (trained_phi.t() @ torch.ones((N_train, 1)).to(self.args.proc))
        #         eval_Dinv = torch.pow(eval_D.flatten(), -1)
        #         self.eval_centering_term = 1 / eval_Dinv.sum() * (trained_phi.t() * eval_Dinv).sum(dim=1)
        #         self.centering_term = self.eval_centering_term
        #     else:
